...
export type Values = ReportFolder | ReportFile;
...
type Props = {
  ...
  addReport: (reportFile: Values) => void;
  editReport: (id: ?string, values: Values) => void;
  addFolder: (values: Values) => void;
  editFolder: (id: ?string, values: Values) => void;
...
export class ReportDetailCMS extends Component {
  state: State;
  props: Props;
        ...
        <Form style={styles.form} onSubmit={handleSubmit}>
          ...
          >
            <MultiSelectItemSidebar
              ...
              icon="group"
              noItemFound="No groups found. Please try another keyword."
...
let formikWrapper = Formik({
  mapPropsToValues: (props: Props) => {
    let {initialValue, type, activeFolder} = props;
    if (type === 'file') {
      return {
        name: (initialValue && initialValue.name) || '',
        linkUrl: (initialValue && initialValue.linkUrl) || '',
        groups: (initialValue && initialValue.groups) || [],
        parentId: (initialValue && initialValue.parentId) || activeFolder,
        createdDate:
          (initialValue && initialValue.createdDate) ||
      ...
      return {
        ...
        groups: (initialValue && initialValue.groups) || [],
        parentId: (initialValue && initialValue.parentId) || activeFolder,
        folders: (initialValue && initialValue.folders) || [],
        files: (initialValue && initialValue.files) || [],
        createdDate:
          (initialValue && initialValue.createdDate) ||
          new Date().toISOString(),
      };
  ...
  validationSchema: (props: Props) => {
    ...
    if (type === 'file') {
      return Yup.object().shape({
        ...
        linkUrl: Yup.string().required(validationMessage.linkUrl.required),
        groups: Yup.array(),
      });
    } else {
      return Yup.object().shape({
  ...
  displayName: 'ReportCMSForm', // helps with React DevTools
});

function mapStateToProps(state: RootState) {
  let {cmsState} = state;
  let {groupCMS, reportCMS} = cmsState;
  return {
    userGroupList: groupCMS.groups,
    activeFolder: reportCMS.activeFolder,
  };
}
...
function mapDispatchToProps(dispatch: Dispatch) {
  return {
    addReport: (reportFile: ReportFile) => {
      dispatch({
        type: 'ADD_REPORT_FILE_REQUESTED',
        reportFile,
      });
    },
    editReport: (fileID: ReportFileID, reportFile: ReportFile) => {
      dispatch({
    ...
    addFolder: (reportFolder: ReportFolder) => {
      dispatch({
        type: 'ADD_REPORT_FOLDER_REQUESTED',
        reportFolder,
      });
    },
    editFolder: (folderID: ReportFolderID, reportFolder: ReportFolder) => {
      dispatch({
